...
def generate_certificates(base_dir,number):
    ...
    for i in range(1,number+1):
        ...
        server_public_file, server_secret_file = zmq.auth.create_certificates(keys_dir, "server%d"%i)
        client_public_file, client_secret_file = zmq.auth.create_certificates(keys_dir, "client%d"%i)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 2 and sys.argv[1]:
        ...
        raise RuntimeError("Please provide the number of nodes to generate certificates for.")

    generate_certificates(os.path.dirname(__file__),n)
